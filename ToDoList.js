...
import { StyleSheet, View, Text, TextInput, TouchableOpacity, CheckBox, Icon } from 'react-native';
...
const Task = ({ task, onComplete, onRemove }) => {
  return (
    <View style={styles.taskBox}>
      ...
      <CheckBox value={task.completed} onValueChange={onComplete} />
    ...
    <View style={[styles.container, isDarkMode && styles.containerDark]}>
      ...
      {tasks.map((task, index) => (
        <Task
          key={index}
          task={task}
          onComplete={() => handleTaskAction(index, 'toggle')}
          onRemove={() => handleTaskAction(index, 'remove')}
        />
      ))}
